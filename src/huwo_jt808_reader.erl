             ...
             rabbit_event:init_stats_timer(
              control_throttle( %% 控制流量?
               #state{socket                 = RealSocket,
                      ...
                      conserve               = false,
                      parse_state            = huwo_jt808_frame:initial_state(),
...
network_error(closed,
    ...
    Fmt = "Huwo JT808 detected network error for ~p: peer closed TCP connection~n",
    Args = [ConnStr],
    case MqttConn of
...
network_error(Reason,
              State = #state{ conn_name  = ConnStr,
                              proc_state = PState }) ->
    rabbit_log_connection:info("Huwo JT808 detected network error for ~p: ~p~n", [ConnStr, Reason]),
    send_will_and_terminate(PState, State).
